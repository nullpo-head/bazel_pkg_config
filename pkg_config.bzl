...
def _extract_pkgconfig_dirs(ctx, files):
    dirs = dict()
    for file in files:
        dirs[str(ctx.path(file).dirname)] = True
    ans = []
    for path in dirs:
        ans.append(path)
    return ans
...
def _pkg_config(ctx, pkg_config, pkg_name, args):
    ...
    if ctx.attr.pkg_config_files:
        pkg_config_path = pkg_config_path + ":".join(_extract_pkgconfig_dirs(ctx, ctx.attr.pkg_config_files))
...
pkg_config = repository_rule(
    attrs = {
        ...
        "pkg_config_files": attr.label_list(doc = ".pc files such that the directories containing them should be added to PKG_CONFIG_PATH.", allow_files = True),
    },
    local = True,
